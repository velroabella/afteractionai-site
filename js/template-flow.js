...
(function() {
  ...
  function generateDebtLetter(d) {
    ...
    if (d.veteranStatus && d.veteranStatus.indexOf('Yes') === 0) {
      sections.push({ heading: 'Veteran Status & SCRA Protections', content: 'I am a United States military ' + d.veteranStatus.replace('Yes — ', '') + '. Many creditors have specific hardship programs for military members and veterans under the Servicemembers Civil Relief Act (SCRA) and similar protections. I respectfully ask that any available military/veteran programs be applied to my account.' });
    }
    sections.push(
      ...
      { heading: 'Important Notice', content: 'This letter is a template starting point and does not constitute legal or financial advice. Results vary by creditor. Sending this letter does not guarantee any specific outcome, approval, or debt modification. Consider consulting a nonprofit credit counselor (NFCC.org) or legal aid for personalized guidance.' }
    );
    return { title: 'Debt Hardship Letter', sections: sections };
  }
  ...
  function generateCreditDispute(d) {
    ...
    sections.push(
      ...
      { heading: 'Enclosures', content: enclosureContent },
      ...
      { heading: 'Important Notice', content: 'This letter follows FCRA dispute guidelines but does not constitute legal advice. Filing a dispute does not guarantee removal or correction of the disputed item. If your situation is complex, consider consulting a consumer rights attorney. Free credit counseling is available through many veteran service organizations.' }
    );
    return { title: 'Credit Dispute Letter', sections: sections };
  }
  ...
  function generateBudgetPlan(d) {
    ...
    return {
      ...
      sections: [
        ...
        { heading: 'Essential Expenses', content: 'Housing (rent/mortgage + utilities): $' + housing.toLocaleString() + ' (' + housingPct + '% of income)' + (housingPct > 30 ? ' [NOTE: Above the commonly recommended 30% guideline]' : ' [Within commonly recommended range]') + '\nTransportation: $' + transport.toLocaleString() + '\nFood/Groceries: $' + food.toLocaleString() + '\n\nTotal Essentials: $' + essentials.toLocaleString() + '\nRemaining after essentials: $' + remaining.toLocaleString() },
        ...
        { heading: 'Debts', content: d.debts + '\n\nCommon Debt Payoff Strategies:\n- Debt snowball: Pay smallest balance first for motivational wins\n- Debt avalanche: Pay highest interest first to minimize total interest\n- Pay minimums on all debts, then put extra toward your target debt\n- Once one debt is paid off, roll that payment into the next' },
        { heading: 'Budget Reference (50/30/20 Guideline)', content: 'A commonly used budgeting framework based on your $' + income.toLocaleString() + ' monthly income:\n\n50% Needs (approx. $' + Math.round(income * 0.5).toLocaleString() + '): Housing, transportation, food, insurance, minimum debt payments\n30% Wants (approx. $' + Math.round(income * 0.3).toLocaleString() + '): Entertainment, dining out, subscriptions, hobbies\n20% Savings & Extra Debt (approx. $' + Math.round(income * 0.2).toLocaleString() + '): Emergency fund, retirement, extra debt payments\n\nNote: These percentages are general guidelines and may not fit every situation. Adjust based on your circumstances.' },
        { heading: 'Action Plan', content: '1. Consider building a starter emergency fund ($500-$1,000)\n2. List all debts and choose a payoff strategy\n3. Review subscriptions and recurring expenses for potential savings\n4. If you have VA disability, confirm you are receiving the correct rating\n5. Check if you qualify for property tax exemptions, vehicle registration discounts, or other veteran benefits in your state\n6. Consider automatic transfers for savings on payday\n7. Review and adjust this budget monthly' },
        ...
        { heading: 'Important Notice', content: 'This budget plan is for educational and planning purposes only. It does not constitute financial advice and does not guarantee any specific financial outcome. Individual circumstances vary. For personalized financial guidance, consult a certified financial counselor. Free counseling is available through Military OneSource (militaryonesource.mil) and the NFCC (nfcc.org).' }
      ]
    };
  }
  ...
  function generateVALoanChecklist(d) {
    ...
    var maxAdditionalPayment = Math.max(0, Math.round(income * 0.41 - debts));
    var dtiStatus = dti <= 41 ? '[GOOD] Within VA 41% DTI guideline' : '[REVIEW] Above 41% — may need compensating factors';

    return {
      title: 'VA Loan Readiness Checklist — ' + d.fullName,
      sections: [
        { heading: 'VA LOAN READINESS CHECKLIST', content: 'Name: ' + d.fullName + '\nState: ' + d.state + '\nDate: ' + new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) },
        { heading: 'Eligibility Status', content: 'Service Status: ' + d.serviceStatus + '\n' + (d.serviceStatus === 'Guard/Reserve (never activated)' ? '[NOTE] Guard/Reserve members who were never activated for 90+ days may have limited VA loan eligibility. Check with the VA.' : '[OK] Your service status typically qualifies for VA loan benefits.') + '\n\nCertificate of Eligibility (COE): ' + d.hasCOE + '\n' + (d.hasCOE === 'Yes' ? '[OK] You have your COE — you\'re ready for this step.' : '[ACTION NEEDED] Request your COE at va.gov/housing-assistance/home-loans/how-to-request-coe or through your lender.') },
        { heading: 'Financial Snapshot', content: 'Monthly Gross Income: $' + income.toLocaleString() + '\nMonthly Debt Payments: $' + debts.toLocaleString() + '\nDebt-to-Income Ratio (DTI): ' + dti + '% ' + dtiStatus + '\nEstimated Additional Monthly Payment Capacity: $' + maxAdditionalPayment.toLocaleString() + ' (based on 41% DTI guideline — this is an estimate, not a loan approval amount)\nSavings for Closing Costs: $' + d.savings },
        { heading: 'Credit Score Assessment', content: 'Your Range: ' + d.creditScore + '\n\n' + (d.creditScore === '740+' ? '[EXCELLENT] Typically qualifies for the most competitive rates.' : d.creditScore === '700-739' ? '[GOOD] Generally no issues qualifying with most lenders.' : d.creditScore === '660-699' ? '[FAIR] Most VA lenders will consider your application. Shop around for the best rates.' : d.creditScore === '620-659' ? '[MARGINAL] VA has no official minimum, but most lenders look for 620+. You may qualify but could see higher rates.' : d.creditScore === '580-619' ? '[BELOW AVERAGE] Some VA lenders may work with you. Consider credit repair first. Use the AfterAction AI Credit Dispute Letter if needed.' : d.creditScore === 'Below 580' ? '[ACTION NEEDED] Consider focusing on credit improvement before applying. Dispute errors, pay down balances, and maintain on-time payments for 6-12 months.' : '[ACTION NEEDED] Check your credit score at annualcreditreport.com (free).') },
        { heading: 'VA Loan Benefits Recap', content: '- NO down payment required (up to conforming loan limits)\n- NO private mortgage insurance (PMI)\n- Competitive interest rates (typically lower than conventional)\n- Limited closing costs (VA limits what veterans can be charged)\n- No prepayment penalties\n- VA funding fee: ' + (d.firstTimeBuyer === 'No — have used VA loan before' ? '3.3% (subsequent use) — can be financed into loan' : '2.15% (first use) — can be financed into loan') + '\n- Funding fee is WAIVED if you have 10%+ VA disability rating\n\nProperty Type: ' + d.propertyType + '\n' + (d.propertyType === 'Condo' ? '[NOTE] The condo must be on the VA-approved list. Check va.gov/gi-bill-comparison-tool.' : d.propertyType === 'Multi-unit (2-4 units)' ? '[NOTE] VA allows up to 4-unit properties if you live in one unit. Rental income from other units can count toward qualifying.' : d.propertyType === 'Manufactured home' ? '[NOTE] Must be on a permanent foundation and meet VA standards.' : '') },
        ...
        { heading: 'Important Notice', content: 'This checklist is for preparation and educational purposes only. It does not constitute a loan approval, pre-qualification, or financial advice. VA loan requirements, rates, and guidelines may change. Actual eligibility depends on lender requirements and current VA policies. Work with a VA-approved lender for current guidelines and official qualification.' }
      ]
    };
  }
  ...
  function generateRentalPacket(d) {
    return {
      ...
      sections: [
        ...
        { heading: 'Important Notice', content: 'This packet helps organize your application materials and does not guarantee application approval. Landlord requirements vary by property and jurisdiction. The information provided is for organizational purposes only and does not constitute legal advice regarding tenant rights or housing law.' }
      ]
    };
  }
